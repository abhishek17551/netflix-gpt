        ...
        <img className='w-40 ' src={BRAND_LOGO} alt='brand-logo'/>
        {
          user && (
            ...
            <div className='flex p-2 m-1'>
              {
                showGptSearch && (
                  <select className='mr-3 mt-2 h-8 rounded-sm bg-black text-white' onChange={handleLanguageChange}>
                  {
              ...
              <button onClick={handleGptSearchClick} className='bg-green-700 font-bold text-white p-1 rounded-md h-10 my-auto hover:bg-green-500'>
                {showGptSearch ? "Home" : "GPT Search"}
              </button>
              <h3 className='font-bold text-white m-2'>{lang[language].greeting}, {user?.displayName} 👋</h3>
